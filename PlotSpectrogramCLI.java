...
public final class PlotSpectrogramCLI extends Application {
	...
	final File wavFile = new File(pargs[0]);
        ...
        /*final Scene scene2 = new Scene(chart_ff, 800, 600);
        scene2.getStylesheets().add("le4music.css");
        final Scene scene_chroma = new Scene(chart_chromagram, 800, 600);
        scene_chroma.getStylesheets().add("le4music.css");
        final Scene scene_chord = new Scene(chart_chord, 800, 600);
        scene_chord.getStylesheets().add("le4music.css");
        
        /* ウィンドウ表示 */
        primaryStage.setScene(scene);
        primaryStage.setTitle(getClass().getName());
        /*secondStage.setScene(scene2);
        secondStage.setTitle(getClass().getName());
        secondStage.setX(900); secondStage.setY(100);
        chromaStage.setScene(scene_chroma);
        chromaStage.setTitle(getClass().getName());
        chromaStage.setX(100); chromaStage.setY(700);
        chordStage.setScene(scene_chord);
        chordStage.setTitle(getClass().getName());
        chordStage.setX(900); chordStage.setY(700);*/
    ...
    // 認識したメロディ＆チャートの表示
    /*public LineChart<Number, Number> makeMelody(CommandLine cmd,
                                                      double sampleRate,
                                                      double nyquist,
                                                      double[] waveform){
        // 1フレームの長さ(sec)とフレームシフト幅(sec)
        ...
        // 基本周波数の候補集合
        double[] ffCandSet = new double[(72 - 35) * 10];
        for(int i = 0; i < ffCandSet.length; i++){
            ffCandSet[i] = 36 + i * 0.1;
        }
        
        // fundFreq[i][j] : フレーム i の音階 j らしさ
        // ノートナンバー = 36 + j * 0.1 (半音を10等分)
